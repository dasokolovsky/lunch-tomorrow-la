...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const totalItems = order.menu_items.reduce((sum: number, item: any) => sum + item.quantity, 0);
    ...
    // Create SMS message
    const message = `🍽️ Order #${orderId} confirmed! ${totalItems} item${totalItems !== 1 ? 's' : ''} ($${totalAmount.toFixed(2)}) for ${new Date(order.order_date).toLocaleDateString()} ${order.delivery_window}. View details: ${orderLink}`;
