...
-- 6. Insert default tip settings (do this before enabling RLS)
...
-- 7. Insert default tax settings (do this before enabling RLS)
...
-- Allow public read access to pricing settings (needed for frontend)
CREATE POLICY "Allow public read access to pricing_fees" ON pricing_fees
  FOR SELECT USING (true);

CREATE POLICY "Allow public read access to tip_settings" ON tip_settings
  FOR SELECT USING (true);

CREATE POLICY "Allow public read access to tax_settings" ON tax_settings
  FOR SELECT USING (true);

CREATE POLICY "Allow public read access to coupon_codes" ON coupon_codes
  FOR SELECT USING (is_active = true);

-- Allow authenticated users to read pricing history
CREATE POLICY "Allow authenticated read access to pricing_history" ON pricing_history
  FOR SELECT USING (auth.role() = 'authenticated');

-- Admin-only write access (you'll need to create an admin role or use service key)
-- For now, we'll allow authenticated users to modify (you can restrict this later)
CREATE POLICY "Allow authenticated write access to pricing_fees" ON pricing_fees
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated write access to tip_settings" ON tip_settings
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated write access to tax_settings" ON tax_settings
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated write access to coupon_codes" ON coupon_codes
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated write access to pricing_history" ON pricing_history
  FOR INSERT USING (auth.role() = 'authenticated');

-- 10. Create trigger function for automatic updated_at timestamps
...
-- 12. Insert some sample data for testing (after RLS is set up)
-- Note: Run these separately after the main setup if you want sample data
